...
/* This program exercises various features of the DE1 Media Computer, as a test and
 * to provide and example. 
 *
 * It performs the following: 
 * 1. tests the SRAM repeatedly
 * 2. scrolls some text on the hex displays, which alternates between "dE1" and
...
_start:
	...
	/* initialize 7-segment displays buffer (dE1 just before being visible on left side) */
...
.endif
	...
	/* toggle display of dE1 and PASSEd */
	movia 	r16, DISPLAY_TOGGLE
	ldw		r17, 0(r16)
...
/* Loop to fill the one-word buffer that drives the parallel port on the DE1 Media
 * Computer connected to the HEX3 to HEX0 displays. The loop produces for each 4-bit
 ...
 * a single 7-seg display in the DE1 Media Computer, for the characters 
 ...
 * these signals to parallel port pins in the DE1 Media Computer.
*/
...
/* HEX3_HEX0 is used to hold the on/off settings for all segments in 
 * the 8 digits of 7-seg *	displays in the DE1 Media Computer. The word is 
 * written to the parallel port that drives HEX3 to HEX0.
*/
HEX3_HEX0:
	.word 0
